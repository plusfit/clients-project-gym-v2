...
export class OnboardingStep3Component implements OnInit {
          ...
          .subscribe({
            ...
            error: async (error) => {
              ...
              const toast = await this.toastCtrl.create({
                message:
                  'No pudimos guardar tus preferencias, pero continuaremos con tu plan',
                duration: 3000,
                position: 'bottom',
  ...
  private assignPlanToUser(loading: HTMLIonLoadingElement) {
    ...
    this.onboardingService.assignPlan().subscribe({
      ...
      error: async () => {
        ...
        const toast = await this.toastCtrl.create({
          message:
            'No se pudo asignar un plan. Se asignará uno predeterminado.',
          duration: 3000,
          position: 'bottom',
          cssClass: 'custom-toast toast-warning',
          buttons: [{ text: 'OK', role: 'cancel' }],
    ...
    if (user) {
      const userInfo = {
        ...
        historyofPathologicalLesions: String(
          user.userInfo.historyofPathologicalLesions,
        ),
      ...
      const mappedUser = {
        ...user,
        role:
          (user.role as keyof typeof UserRole) in UserRole
  ...
  private navigateToPlan() {
    ...
    this.navCtrl.navigateRoot('/cliente/mi-plan', {
