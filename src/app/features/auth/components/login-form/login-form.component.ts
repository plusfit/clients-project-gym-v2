...
export class LoginFormComponent implements OnDestroy {
  ...
  submit() {
    if (this.form.valid) {
      ...
      this.actions
        .pipe(ofActionSuccessful(Login), takeUntil(this._destroyed))
        .subscribe(() => {
      ...
      this.actions
        .pipe(ofActionErrored(Login), takeUntil(this._destroyed))
        .subscribe(() => {
          this.isLoading = false;
        });
    } else {
  ...
  registerWithGoogle() {
    ...
    this.actions
      .pipe(ofActionSuccessful(GoogleLogin), takeUntil(this._destroyed))
      .subscribe(() => {
        this.isLoading = false;
        const user = this.store.selectSnapshot((state) => state.auth.user);
        if (user) {
    ...
    this.actions
      .pipe(ofActionErrored(GoogleLogin), takeUntil(this._destroyed))
      .subscribe(() => {
        this.isLoading = false;
      });
  }
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: (data) => {
            ...
            this.actions
              .pipe(
                ofActionSuccessful(ForgotPassword),
            ...
            this.actions
              .pipe(ofActionErrored(ForgotPassword), takeUntil(this._destroyed))
              .subscribe(() => {
                this.isLoading = false;
              });
            return true;
